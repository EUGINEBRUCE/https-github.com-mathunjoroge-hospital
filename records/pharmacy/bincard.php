...
<?php 
        //get quantities     
        $result = $db->prepare("SELECT* FROM orders WHERE updated_at BETWEEN :a AND :b AND drug_id=:c");
...
<thead>
<tr>
  <?php $result = $db->prepare("SELECT sum(qty_disp) AS total FROM orders WHERE updated_at BETWEEN :a AND :b AND drug_id=:c");
        $result->bindParam(':a',$date1);
        $result->bindParam(':b',$date2);
        $result->bindParam(':c',$drug);       
  $result->execute();
...
<?php 
//get quantities     
        $result = $db->prepare("SELECT* FROM purchases WHERE date BETWEEN :a AND :b AND drug_id=:c");
        $result->bindParam(':a',$date1);
        $result->bindParam(':b',$date2);
        $result->bindParam(':c',$drug);       
  $result->execute();
...
<thead>
<tr>
  <?php $result = $db->prepare("SELECT sum(qty) AS total FROM purchases WHERE date BETWEEN :a AND :b AND drug_id=:c");
        $result->bindParam(':a',$date1);
        $result->bindParam(':b',$date2);
        $result->bindParam(':c',$drug);       
  $result->execute();
  for($i=0; $row = $result->fetch(); $i++){
      ...
      $total = $row['total'];
       ...
       <?php 
       $reset=1;
       $result = $db->prepare("SELECT sum(quantity) AS dis_total FROM dispensed_drugs WHERE date BETWEEN :a AND :b AND drug_id=:c AND dispensed=:d");
        $result->bindParam(':a',$date1);
        $result->bindParam(':b',$date2);
        $result->bindParam(':c',$drug);
