...
<tr>  <th>disease code</th>
 ...
 <tbody>
  ...
  $result = $db->prepare("SELECT disease,title,count(disease) AS total FROM ddx RIGHT OUTER JOIN  icd_second_level_codes ON ddx.disease=icd_second_level_codes.code  WHERE date(date)>=:a AND date(date)<=:b");
