...
?>
  ...
  <div class="content-wrapper">
      ...
      <div class="container" ><label>record purchase</label></br>
        <form action="savepurchases_lab.php" method="POST">
...
<?php 
      ...
      <input name="qty" style="height: 2em;width:3em;" type="number" min="1" placeholder="qty"><input name="cost" style="height: 2em;width:3em;" type="number" min="1" placeholder="cost"><button class="btn btn-success btn-large">add</button></form></span></div>
...
<?php
       $invoice=$_GET['req'];
        $result = $db->prepare("SELECT reagents.drug_id,generic_name,brand_name,lab_purchases.price AS price,lab_purchases.qty FROM lab_purchases RIGHT OUTER JOIN reagents ON reagents.drug_id=lab_purchases.drug_id WHERE inv='$invoice'");
  $result->execute();
  for($i=0; $row = $result->fetch(); $i++){
...
<tr> <?php $invoice=$_GET['req'];
        $result = $db->prepare("SELECT sum(lab_purchases.price*lab_purchases.qty) as total FROM lab_purchases RIGHT OUTER JOIN reagents ON reagents.drug_id =lab_purchases.drug_id WHERE inv='$invoice'");
  $result->execute();
  for($i=0; $row = $result->fetch(); $i++){ ?>
      <th> </th>
      <th>  </th>
 ...
 <a rel="facebox" href="checkout_lab.php?invoice=<?php echo $invoice; ?>&total=<?php echo $row['total']; ?>">
