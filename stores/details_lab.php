...
<?php 
        $patient=$_GET['id'];
        $result = $db->prepare("SELECT reagents.drug_id,generic_name,brand_name,price,dispense_id,lab_orders.quantity,qty_disp FROM lab_orders RIGHT OUTER JOIN reagents ON reagents.drug_id=lab_orders.drug_id WHERE patient='$patient' AND dispensed=1");
  $result->execute();
...
<tr> <?php $patient=$_GET['id'];
        $result = $db->prepare("SELECT sum(price*lab_orders.quantity) as total FROM lab_orders RIGHT OUTER JOIN reagents ON reagents.drug_id =lab_orders.drug_id WHERE patient='$patient' AND dispensed=1");
  $result->execute();
  for($i=0; $row = $result->fetch(); $i++){ ?>
      <th> </th>
      <th>  </th>
