...
<?php  
$result = $db->prepare("SELECT DISTINCT(name),age,sex,opno FROM patients RIGHT OUTER JOIN lab ON lab.opn=patients.opno WHERE lab.served=0");
  $result->execute();
...
<?php  
$result = $db->prepare("SELECT name,age,sex,opno FROM patients RIGHT OUTER JOIN  req_images ON  req_images.opn=patients.opno WHERE  req_images.served=10");
  $result->execute();
  for($i=0; $row = $result->fetch(); $i++){
