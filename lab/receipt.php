...
<?php
       ...
       $cashier=$_SESSION['SESS_FIRST_NAME'];
        $result = $db->prepare("SELECT drugs.drug_id,generic_name,brand_name,price,dispense_id,dispensed_drugs.quantity FROM dispensed_drugs RIGHT OUTER JOIN drugs ON drugs.drug_id=dispensed_drugs.drug_id WHERE patient='$patient' AND `dispensed`='1'");
  $result->execute();
  for($i=0; $row = $result->fetch(); $i++){
...
$cashier=$_SESSION['SESS_FIRST_NAME'];
        $result = $db->prepare("SELECT sum(price*dispensed_drugs.quantity) as total FROM dispensed_drugs RIGHT OUTER JOIN drugs ON drugs.drug_id =dispensed_drugs.drug_id WHERE patient='$patient' AND `dispensed`='1'");
  $result->execute();
  for($i=0; $row = $result->fetch(); $i++){ ?>
      <th> </th>
      ...
      <tr>
        <th colspan="3"><strong style="font-size: 12px; color: #222222;">cash tendered </strong></th>
        <td colspan="1"><strong style="font-size: 12px; color: #222222;"> <?php   echo $_GET['cash']; ?></strong> </td>
        </tr>
        <tr>
        <th colspan="3"><strong style="font-size: 12px; color: #222222;">change: </strong></th>
        <td colspan="1"><strong style="font-size: 12px; color: #222222;"> <?php   echo $_GET['cash']-$amount; ?></strong> </td>
        </tr>
